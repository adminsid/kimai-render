...
# Run the slow Doctrine database initialization and symfony cache generation in the background!
# This prevents Cloudflare Containers from timing out the port readiness check at 60s!
(
  waitForDB
  prepareKimai
  /opt/kimai/bin/console kimai:reload --env="$APP_ENV"
...
if [ -e /use_apache ]; then
  # Apache strips environment variables by default. Add PassEnv for our split TiDB SSL variables!  
  sed -i '/PassEnv DATABASE_URL/a \    PassEnv DATABASE_HOST\n    PassEnv DATABASE_PORT\n    PassEnv DATABASE_USER\n    PassEnv DATABASE_PASSWORD\n    PassEnv DATABASE_NAME' /etc/apache2/sites-enabled/000-default.conf
